...
for f in index.html assets/style.css assets/data.js assets/app.js assets/scene.js assets/vendor/three.module.min.js; do
  mkdir -p "$W/$(dirname "$f")"
  curl -fsSL "$B/$f?$(date +%s)" -o "$W/$f"
done
chown -R imezon:imezon "$W/index.html" "$W/assets"
